...
export const selectReleasesFeatureState =
  createFeatureSelector<ReleasesState>('releases');

export const selectReleases = createSelector(
  selectReleasesFeatureState,
  (state: ReleasesState) => state.releases
);
...
export const selectFutureReleases = createSelector(
  ...
  (state: ReleasesState) => {
    ...
    return state.releases
      ...
      .sort(ReleasesService.sortByDateDescending);
  }
);

export const selectPastReleases = createSelector(
  selectReleasesFeatureState,
  (state: ReleasesState) => {
    const today = new Date();
    return state.releases
      .filter((release) => release.releaseDate <= today)
      .sort(ReleasesService.sortByDateAscending);
  }
);
...
export const selectReleasesForPress = createSelector(
  selectReleasesFeatureState,
  (state: ReleasesState) =>
